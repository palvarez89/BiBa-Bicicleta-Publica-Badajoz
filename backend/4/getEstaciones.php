<?php
 error_reporting (5); 
	...
	echo "Failed to load http://www.bicibadajoz.es/estado/EstadoactualBis.asp";
        die();
    }
   ...
   while ($i < $items->length) {
		...
		$json = array_merge($json,array(array('n' => $numero, 'name' => $nombre,'lat'=> $lat[$numero-1],'lon'=> $long[$numero-1], 'state' => $estado, 'avail' => filter_var($disp, FILTER_SANITIZE_NUMBER_INT), 'total' => filter_var($capac, FILTER_SANITIZE_NUMBER_INT))));
